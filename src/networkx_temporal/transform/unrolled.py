...
def from_unrolled(UTG: StaticGraph) -> TemporalGraph:
    ...
    .. attention::
        ...
        Graph-level dynamic attributes are not preserved in the resulting object.
    ...
    for edge in UTG.edges(**edge_kwargs):
        ...
        assert u_t.isdigit(),\
            f"Unrolled temporal graph (UTG) contains non-temporal nodes ('{u}')."
        assert v_t.isdigit(),\
            f"Unrolled temporal graph (UTG) contains non-temporal nodes ('{v}')."
...
def to_unrolled(
    ...
    :math:`(u_{t}, v_{t+\delta})`, resulting in time series representations [1]_ of temporal
    networks.
    ...
    assert delta is None or type(delta) == int and delta >= 0,\
        f"Argument `delta` must be a non-negative integer."
    assert node_copies in (None, "all", "fill", "persist"),\
        "Invalid value for `node_copies`, expect one of ('all', 'fill', 'persist')."
