...
def from_events(
    ...
    # (u, v, t, e), where e is a float (positive)
...
def to_events(
    ...
    :param str attr: Edge attribute to consider when ``delta`` is ``'float'``. If provided, the
    ...
    assert delta in (None, int, float),\
        f"Argument `delta` must be either `int` or `float` if provided."
