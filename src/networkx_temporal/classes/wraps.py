...
def copy(self: TemporalGraph, as_view: bool = False) -> TemporalGraph:
    ...
    TG.graphs.extend([G.copy(as_view=as_view) for G in self])
    TG.name = self.name
    TG.names = self.names
    return TG
copy.__doc__ += "\n:meta private:"
...
def to_directed(self: TemporalGraph, as_view: Optional[bool] = True) -> TemporalGraph:
    ...
    TG.graphs.extend([G.to_directed(as_view=as_view) for G in self])
    TG.name = self.name
    TG.names = self.names
    return TG
to_directed.__doc__ += "\n:meta private:"
...
def to_undirected(self: TemporalGraph, as_view: Optional[bool] = True) -> TemporalGraph:
    ...
    TG.graphs.extend([G.to_undirected(as_view=as_view) for G in self])
    TG.name = self.name
    TG.names = self.names
    return TG
