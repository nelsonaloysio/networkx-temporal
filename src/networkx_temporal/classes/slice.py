...
def _slice(
    ...
    contain temporal node copies, provided their interactions fall into different intervals.
    ...
    :param intervals: List of time intervals for slicing. If unset, the method will create
        as many snapshots as unique values found in the specified node- or edge-level ``attr``
        attribute data.
    ...
    if intervals is not None and not all(type(i) == tuple and len(i) == 2 for i in intervals):
        raise ValueError("Argument `intervals` must be a list of 2-tuple elements.")
    if bins is not None and intervals is not None:
        raise ValueError("Arguments `bins` and `intervals` are mutually exclusive.")
    ...
    # Create time intervals from user-defined tuples, or default to unique time values.
