...
class TemporalABC(metaclass=ABCMeta):
    ...
    def __getitem__(self, t: Union[str, int, slice]) -> StaticGraph:
        ...
        if type(t) not in (str, int, slice):
            raise TypeError(f"Indices must be of type str, int, or slice, received: {type(t)}.")
    ...
    @property
    def edge(self) -> Optional[List[str]]:
        """ The ``edge`` property of the temporal graph.

        Returns a dictionary mapping each edge to another dictionary
        with snapshot indices corresponding to edge attribute dictionaries.
        """
        dct = {e: {} for e in self.temporal_edges()}
    ...
    @property
    def node(self) -> Optional[List[str]]:
        """ The ``node`` property of the temporal graph.

        Returns a dictionary mapping each node to another dictionary
        with snapshot indices corresponding to node attribute dictionaries.
        """
        dct = {n: {} for n in self.temporal_nodes()}
        for t, G in enumerate(self):
            for n in dct:
                if n in G.nodes:
                    dct[n][t] = G.nodes[n]
    ...
    def add_snapshot(self, n: int = 1) -> None:
        """ Adds one or more empty snapshots to the temporal graph.

        :param n: Number of empty snapshots to add.
        """
        for _ in range(n):
    ...
    def index_edge(self, edge: tuple, interval: Optional[range] = None) -> list:
        ...
        """
        if not (interval is None or type(interval) in (range, tuple, list)):
    ...
    def is_directed(self) -> bool:
        """ Returns ``True`` if the temporal graph is directed, ``False`` otherwise.
        """
        return bool(self.__dict__.get("_directed", None))

    def is_multigraph(self) -> bool:
        """ Returns ``True`` if the temporal graph is a multigraph, ``False`` otherwise.
        """
