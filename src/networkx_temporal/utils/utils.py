...
def get_edge_attributes(TG: TemporalGraph, attr: str, index: bool = False) -> List[Any]:
    """ Returns edge attribute values from all snapshots.

    :param TG: :class:`~networkx_temporal.classes.TemporalGraph` object.
    :param attr: The edge attribute key.
    :param index: If ``True``, returns a dictionary with edges as keys.
    """
    if not is_temporal_graph(TG):
        raise TypeError("Argument `TG` must be a temporal NetworkX graph.")

    edge_attr = [nx.get_edge_attributes(G, attr) for G in TG]
    if not index:
        edge_attr = [list(edge_attr.values()) for edge_attr in edge_attr]
    return edge_attr

def get_node_attributes(TG: TemporalGraph, attr: str, index: bool = False) -> List[Any]:
    """ Returns node attribute values from all snapshots.

    :param TG: :class:`~networkx_temporal.classes.TemporalGraph` object.
    :param attr: The node attribute key.
    :param index: If ``True``, returns a dictionary with nodes as keys.
    """
    if not is_temporal_graph(TG):
        raise TypeError("Argument `TG` must be a temporal NetworkX graph.")

    node_attr = [nx.get_node_attributes(G, attr) for G in TG]
    if not index:
        node_attr = [list(node_attr.values()) for node_attr in node_attr]
    return node_attr


def get_unique_edge_attributes(TG: TemporalGraph, attr: str) -> List[Any]:
...
def get_unique_node_attributes(TG: TemporalGraph, attr: str) -> List[Any]:
    ...
    values = set()
    for G in TG:
        values.update(nx.get_node_attributes(G, attr).values())
    return list(values)


def edge_attributes_from_nodes(
...
def node_attributes_from_edges(
    TG: Union[TemporalGraph, StaticGraph],
    edge_attr: str,
    unique: bool = False,
) -> list:
