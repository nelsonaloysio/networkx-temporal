...
def dynamic_stochastic_block_model(
    ...
    :param sparse: Use sparse instead of dense matrices. Default is ``False``.
    ...
    if d is not None and (len(z) != len(d) or (d_out is not None and len(d) != len(d_out))):
        raise ValueError("Length of communities `z` differs from degree vectors `d` or `d_out`.")
    ...
    for ti in range(t or 1):
        ...
        nx.set_node_attributes(G, {n: c for n, c in zip(G.nodes(), communities)}, "community")
