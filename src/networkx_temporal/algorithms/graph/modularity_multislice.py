...
from .modularity import modularity, _node_attr_to_dict
...
def multislice_modularity(
    ...
    Note that if ``sparse=True``, modularity is computed using
    ...
    labels = [
        _node_attr_to_dict(G, communities[t] if type(communities[0]) == list else communities)
