...
def degree_centrality(
    ...
    :math:`c_{\\text{deg}}` is the degree centrality,
    :math:`G \\in \\mathcal{G}` is a snapshot in the temporal graph :math:`\\mathcal{G}`,
    :math:`j` is a node in the neighborhood :math:`\\mathcal{N}(i)`,
    :math:`\\mathbf{A}` is the adjacency matrix,
    and :math:`|\\mathcal{V}|` is the number of nodes.
...
def in_degree_centrality(
    ...
    where :math:`i` is a node,
    :math:`c_{\\text{in}}` is the degree centrality,
    :math:`G \\in \\mathcal{G}` is a snapshot in the temporal graph :math:`\\mathcal{G}`,
    :math:`j` is a node in the neighborhood :math:`\\mathcal{N}(i)`,
    :math:`\\mathbf{A}` is the adjacency matrix,
    and :math:`|\\mathcal{V}|` is the number of nodes.
