...
from ..utils import convert, _dict
...
class TemporalABC(metaclass=ABCMeta):
    ...
    temporal_in_degree = in_degree
    temporal_out_degree = out_degree
    to_events = to_events
    ...
    def __init__(self, t: int, create_using: StaticGraph):
        ...
        # NOTE: fixes inheritance issues (MultiDiGraph is a subclass of MultiGraph).
        inherits = (nx.MultiGraph if create_using == nx.MultiDiGraph else create_using)

        # Inherit methods from NetworkX graph.
        for name in inherits.__dict__.keys():
            if not name.startswith("__") and TemporalABC.__dict__.get(name, None) is None:
                try:
                    self.__setattr__(name, wrapper(self, name))
    ...
    def temporal_order(self) -> int:
        ...
        Equivalent to `:func:`networkx_temporal.metrics.graph.order```(copies=False)``.
        """
    ...
    def temporal_size(self) -> int:
        ...
        Equivalent to `:func:`networkx_temporal.metrics.graph.size```(copies=False)``.
        """
        return self.size(copies=False)

    def total_order(self) -> int:
        """
        Return sum of nodes in all snapshots.

        Equivalent to `:func:`networkx_temporal.metrics.graph.order```(copies=True)``.
        """
        return self.order(copies=True)

    def total_size(self) -> int:
        """
        Return sum of edges in all snapshots.

        Equivalent to `:func:`networkx_temporal.metrics.graph.size```(copies=True)``.
        """
