...
def write_graph(
    ...
    **kwargs
    ...
    frmt = _get_format(path, frmt) or DEFAULT_FORMAT
    ...
    if type(file) == TextIOWrapper:
        raise TypeError(
            f"File must be opened in binary mode, received {type(file)} "
    ...
    if frmt is None:
        raise RuntimeError(
            "Format could not be determined from file name or `frmt` parameter."
        )
