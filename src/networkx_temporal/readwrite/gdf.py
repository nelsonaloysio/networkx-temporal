...
def read_gdf(
    ...
    """
    Returns a NetworkX graph from GDF file or object.
    ...
    try:
        ...
        raise ImportError(
            "The `networkx-gdf` package is required to read and write GDF files. "
            "Please install it via `pip install networkx-gdf`."
        ) from e
...
def write_gdf(
    ...
    """
    Writes a NetworkX graph to GDF file or object.
    ...
    try:
        ...
        raise ImportError(
            "The `networkx_gdf` package is required to read and write GDF files. "
            "Please install it via `pip install networkx_gdf`."
        ) from e

    if is_temporal_graph(graph):
        raise ValueError(
            "Input expects a static graph; please use `write_graph` for temporal graphs.")
    if not is_static_graph(graph):
        raise ValueError("Input graph must be a static graph or a temporal graph.")
