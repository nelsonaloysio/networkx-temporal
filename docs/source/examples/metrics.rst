...
Algorithms available from NetworkX may be called directly on each graph snapshot, and methods
available from a
`NetworkX graph <https://networkx.org/documentation/stable/reference/classes/graph.html#networkx.Graph>`__
are inherited by
:class:`~networkx_temporal.graph.TemporalGraph` instances.
This section showcases a few common examples of use, including graph-level and node-level
metrics.
...
The methods
:func:`~networkx_temporal.graph.TemporalGraph.order`
and :func:`~networkx_temporal.graph.TemporalGraph.size`
return the number of nodes and edges in each graph snapshot, respectively,
while an additional argument ``copies`` allows to specify whether to count
the number of unique nodes and edges or the total number of nodes and edges
(including copies) in the graph:
...
.. code-block:: python
   ...
   Size: [1, 1, 3, 3]
   Size (unique edges): 8
   Size (including copies): 8

Note that when printing a :class:`~networkx_temporal.graph.TemporalGraph` instance, the order of
...
.. seealso::
   ...
   The alias methods:
   :func:`~networkx_temporal.graph.TemporalGraph.temporal_order`,
   :func:`~networkx_temporal.graph.TemporalGraph.temporal_size`,
   :func:`~networkx_temporal.graph.TemporalGraph.total_order`,
   and :func:`~networkx_temporal.graph.TemporalGraph.total_size`.
...
.. note::
   ...
   The graph in- and out-degree centralitization are not always equal in directed graphs:
...
.. seealso::
   ...
   The alias methods
   :func:`~networkx_temporal.graph.TemporalGraph.total_degree`,
   :func:`~networkx_temporal.graph.TemporalGraph.total_in_degree`,
   :func:`~networkx_temporal.graph.TemporalGraph.total_out_degree`,
   which all return a dictionary with the sum of node degrees over time,
...
The :func:`~networkx_temporal.graph.TemporalGraph.neighbors`
method returns a generator for each snapshot, respecting edge direction:
...
The :func:`~networkx_temporal.algorithms.neighbors` function instead
returns node neighbors in all snapshots:
